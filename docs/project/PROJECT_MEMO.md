...
- `Q` 键启动的自动发射位于 `AutoShootController`：预热、定时送一发、可选
  间隔、冷却后回到手动 `ShootCmd`。遥控掉线会复位自动状态机并保留零输出。
  当前没有摩擦轮达速或拨盘单发位置的标准反馈接口，故预热、送弹和间隔均是
  必须经实车标定的毫秒参数，默认仅一发，不能宣称为闭环单发。
...
- 2026-09-02：本机没有 `arm-none-eabi-gcc`/STARM 标准库，未完成整固件 ARM
  编译链接；不得据此直接判定可烧录。
- 2026-09-17：自动发射状态机主机测试覆盖 Q 键沿触发、预热、送弹、冷却和
  回到空命令；完整主机测试脚本通过。
